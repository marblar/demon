...
// TODO: Encapsulate me!
typedef std::map<char,DemonBase::SystemState *> TransitionTable;
typedef std::map<DemonBase::SystemState *, TransitionTable> TransitionRule;
...
namespace Ising {
    ...
    public:
        ...
        public:
            virtual Reservoir *create(gsl_rng *RNG, DemonBase::Constants constants);
            Factory(int dim, int clst) : dimension(dim), clusters(clst) {}
        };
        int clusters;
        virtual void reset();
        gsl_rng *RNG;
